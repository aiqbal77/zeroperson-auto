...
function initializeTables() {
 ...
 console.log("✅ Database tables initialized (including users table for authentication)");
 });
}
...
async function checkAndSeedSupabase() {
 ...
 // Check users table exists and warn if empty
 const { data: usersData, error: usersErr } = await supabase.from('users').select('id').limit(1);
 if (usersErr && usersErr.code === '42P01') {
 console.warn("⚠️  Supabase users table not found! Run the SQL to create it.");
 console.log(`
...
app.post('/api/crm/:tenant', requireTenantAccess, async (req, res) => {
 const tenant = req.params.tenant;
...
app.patch('/api/crm/:tenant/:id', requireTenantAccess, async (req, res) => {
...
// 4. Delete Lead
app.delete('/api/crm/:tenant/:id', requireTenantAccess, async (req, res) => {
 const { tenant, id } = req.params;
...
app.post('/api/teasers/unlock/:tenant', requireTenantAccess, async (req, res) => {
 const tenant = req.params.tenant;
 const { id } = req.body;
...
// 1. Get all active tenants (all authenticated users)
...
// 2. Provision new tenant (super-admin only)
app.post('/api/tenants', requireRole('super-admin'), async (req, res) => {
...
// 3. Delete tenant (super-admin only)
app.delete('/api/tenants/:id', requireRole('super-admin'), async (req, res) => {
 const { id } = req.params;
...
// ==========================================
// VOICE WEBHOOK (Public - called by Vapi)
// ==========================================
